...
map <-
  ...
  addPolygons(data = demographics, group = "Neighborhood Demographics<hr>", fillColor = ~demo_pal(pct_api_clean), color = gray, weight = 1, fillOpacity = .8) %>%
  
  # legends
  
  addLegend(position = "bottomright", pal = demo_pal, values = demographics$pct_api_clean, opacity = .8, title = "% Asian or Pacific Islander<br>ACS 2019 (5-year)", group = "Neighborhood Demographics<hr>")
  ...
  walk(function(x)                         # then walk through vector of names one at a time
    map <<-
      ...
      addMarkers(data = food_resources %>% filter(category == x),
                 icon = makeIcon(iconUrl = ~icon_url, iconWidth = 10, iconHeight = 30),
                 popup = ~popup,
                 group = x,
...
map <-
  map %>%
  addLayersControl(
    overlayGroups = c("Neighborhood Demographics<hr>",  groups),
    options = layersControlOptions(collapsed = FALSE,
                                   postiton = "topright")
  ) %>%
  hideGroup("Neighborhood Demographics<hr>")
