...
food_resources <- read_rds("data/full_dataset.rds") %>%
  ...
  mutate(category = factor(category),
         icon_url = case_when(category == "Corner Stores" ~ "assets/corner-stores.svg",
                      ...
                      category == "Farmers Markets" ~ "assets/farmers-markets.svg",
                      TRUE ~ NA_character_
  )
  )
...
map <leaflet() %>%
  ...
  addPolygons(data = demographics, group = "Neighborhood Demographics<hr padding:0px;margin:0px;>", fillColor = ~demo_pal(pct_api_category), color = gray, weight = 1, fillOpacity = .8, popup = ~popup) %>%

  # legends

  addLegend(position = "bottomright", pal = demo_pal, values = demographics$pct_api_category, opacity = .8, title = "% Asian or Pacific Islander<br><span style='font-weight:700; font-size:12px;'>ACS 2019 (5-year), quantile breaks", group = "Neighborhood Demographics<hr padding:0px;margin:0px;>", na.label = "Data not available", labels = c("Less than 16%", "16 to 22%", "22 to 34%", "34 to 44%", "Greater than 44%"))
